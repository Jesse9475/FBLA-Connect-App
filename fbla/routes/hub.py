...
def hub_collection():
    ...
    if request.method == "GET":
        ...
        return jsonify({"hub_items": result.data})
    ...
    return jsonify({"hub_item": result.data[0] if result.data else cleaned}), 201
...
def hub_detail(item_id):
    ...
    return jsonify({"hub_item": result.data[0] if result.data else cleaned})
